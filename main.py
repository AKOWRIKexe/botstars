...
BOT_USERNAME = None
STICKER_ID = "CAACAgIAAxkBAAIKs2kaECJBZidAZpGPHm_xWXqzdrpCAAInFQACfrVBSL_MzNpIHz70NgQ"
...
def send_welcome(message):
    ...
    try:
        ...
        if not check_subscription(user_id):
            show_subscription_message(message.chat.id)
            return
        ...
        if not users_table.search(User.user_id == user_id):
            ...
            if referrer_id and referrer_id != str(user_id):
                ...
                if referrer and not referrer[0].get('frozen', False):
                    ...
                    users_table.update({'balance': new_balance, 'referrals': referrer_data['referrals']},
                                       User.user_id == int(referrer_id))
                    bot.send_message(referrer_id,
                                     f"Новый пользователь по твоей ссылке! +{reward} звезд ⭐️\nТвой баланс: {new_balance} звезд ⭐️",
                                     parse_mode='HTML')
...
def show_subscription_message(chat_id):
    try:
        ...
        bot.send_message(chat_id, "Перед использованием бота подпишитесь на каналы:", reply_markup=markup)
    except Exception as e:
...
def show_main_menu(chat_id):
    try:
        markup = types.InlineKeyboardMarkup()
        btn_referral = types.InlineKeyboardButton("[🌠] ЗВЕЗДНЫЙ БАНК", callback_data="referral")
        btn_profile = types.InlineKeyboardButton("[👥] МОЙ ЭКИПАЖ", callback_data="profile")
        btn_withdraw = types.InlineKeyboardButton("[ 🪐 💫 ВЫВЕСТИ ЗВЕЗДЫ 🚀 ]", callback_data="withdraw")
        btn_tasks = types.InlineKeyboardButton("[🛰️] МОИ МИССИИ", callback_data="tasks_1")
        btn_promo = types.InlineKeyboardButton("[ 🌠 🎫 ПРОМОКОД 🔮 ]", callback_data="promo")
        btn_top = types.InlineKeyboardButton("[🏆] ГАЛАКТИЧЕСКИЙ ТОП", callback_data="top_day")
...
def admin_panel(message):
    ...
    try:
        ...
        btn_set_ref_reward = types.InlineKeyboardButton("Установить награду за реферала",
...
def handle_admin_input(message):
    ...
    try:
        if step == 'channel_id':
            ...
            admin_commands_table.update({'channel_id': message.text, 'step': 'task_link'},
                                        AdminCommand.user_id == user_id)
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("Отмена", callback_data="admin_cancel"))
            bot.reply_to(message, "Введите ссылку на канал:", reply_markup=markup)
        ...
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("Отмена", callback_data="admin_cancel"))
        bot.reply_to(message, "Ошибка! Введи корректное значение.", reply_markup=markup)
    except Exception as e:
...
def handle_promo_input(message):
    ...
    try:
        ...
        bot.reply_to(message,
                     f"Промокод активирован! Ты получил {promo['reward']} звезд ⭐️\nТвой баланс: {new_balance} звезд ⭐️")
    except Exception as e:
        print(f"Error in handle_promo_input: {e}")
        restart_bot()
    ...
    try:
        if call.data == "check_subscription":
            if check_subscription(user_id):
                bot.edit_message_text("<b>Главное меню</b>", chat_id=call.message.chat.id,
                ...
                bot.answer_callback_query(call.id, "Вы не подписаны на все обязательные каналы!")
            return
        ...
        user_data = users_table.search(User.user_id == user_id)[0]
        ...
        if call.data.startswith("withdraw_sent_") or call.data.startswith("withdraw_denied_"):
            ...
            public_text = f"Заявка №{withdrawal_id}\nПользователь: @{withdrawal['username']} | ID: {user_id}\nКоличество: {stars} звезд ⭐️\nСтатус: {status}"
            bot.edit_message_text(chat_id=config.PUBLIC_CHANNEL_ID, message_id=public_msg_id, text=public_text)

            admin_text = f"Заявка №{withdrawal_id}\nПользователь: @{withdrawal['username']} | ID: {user_id}\nКоличество: {stars} звезд ⭐️\nСтатус: {status}"
            bot.edit_message_text(chat_id=config.ADMIN_CHANNEL_ID, message_id=call.message.message_id, text=admin_text)

            if action == "sent":
                bot.send_message(user_id, f"Заявка №{withdrawal_id} выполнена!")
            else:
                bot.send_message(user_id, f"Заявка №{withdrawal_id} отклонена.")
        ...
        if call.data == "profile":
            ...
            withdrawals_count = len(
                [w for w in withdrawals_table.search(User.user_id == user_id) if w['status'] == 'Звезды отправлены'])
            profile_text = (
                "<b>Мой профиль</b>\n\n"
                f"ID: {user_id}\n"
            ...
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=profile_text,
            ...
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=referral_text,
                                  reply_markup=markup, parse_mode='HTML')
            ...
            btn_next = types.InlineKeyboardButton(">", callback_data=f"tasks_{page + 1}" if page < len(
                available_tasks) else "none")
            ...
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=task_text,
                                  reply_markup=markup, parse_mode='HTML')
            ...
            if task_id in completed_tasks:
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                      text="Ты уже выполнил это задание!")
                return

            if user_data.get('frozen', False):
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                      text="Твой аккаунт заморожен, выполнение заданий невозможно!")
                return
            ...
            if chat_member.status in ['member', 'administrator', 'creator']:
                ...
                users_table.update({'balance': new_balance, 'completed_tasks': completed_tasks},
                ...
                if available_tasks:
                    ...
                    btn_next = types.InlineKeyboardButton(">", callback_data="tasks_2" if len(
                        available_tasks) > 1 else "none")
                    markup.row(btn_prev, btn_page, btn_next)
                    ...
                    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                          text=next_task_text, reply_markup=markup, parse_mode='HTML')
                else:
                    markup = types.InlineKeyboardMarkup()
                    markup.add(types.InlineKeyboardButton("Назад", callback_data="cancel"))
                    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
            ...
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                  text="Введите промокод для активации:", reply_markup=types.InlineKeyboardMarkup().add(
                    types.InlineKeyboardButton("Назад", callback_data="cancel")))
            ...
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=withdraw_text,
                                  reply_markup=markup, parse_mode='HTML')
            ...
            if user_data.get('frozen', False):
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                      text="Твой аккаунт заморожен, вывод невозможен!")
                return
            ...
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                  text=f"Заявка №{withdrawal_id} на {stars} звезд ⭐️ создана!")

            public_text = f"Заявка №{withdrawal_id}\nПользователь: @{user_data['username']} | ID: {user_id}\nКоличество: {stars} звезд ⭐️\nСтатус: Ожидание"
            public_msg = bot.send_message(config.PUBLIC_CHANNEL_ID, public_text)

            admin_text = f"Заявка №{withdrawal_id}\nПользователь: @{user_data['username']} | ID: {user_id}\nКоличество: {stars} звезд ⭐️\nСтатус: Ожидание"
            markup = types.InlineKeyboardMarkup()
            btn_sent = types.InlineKeyboardButton("Отправлено",
                                                  callback_data=f"withdraw_sent_{withdrawal_id}_{public_msg.message_id}")
            btn_denied = types.InlineKeyboardButton("Отказано",
                                                    callback_data=f"withdraw_denied_{withdrawal_id}_{public_msg.message_id}")
            markup.add(btn_sent, btn_denied)
            admin_msg = bot.send_message(config.ADMIN_CHANNEL_ID, admin_text, reply_markup=markup)
            ...
            for user in all_users:
                ...
                recent_referrals = [r for r in referrals if now - datetime.fromisoformat(
                    users_table.search(User.user_id == r)[0]['registration_date']) <= time_delta]
                if recent_referrals:
                    top_users.append(
                        (f"{user['first_name']} {user['last_name']}".strip(), len(recent_referrals), user['user_id']))

            top_users.sort(key=lambda x: x[1], reverse=True)
            top_5 = top_users[:5]

            top_text = f"<b>{title}</b>\n\n" + "\n".join([f"{i + 1}. {name} - {ref} друзей" for i, (name, ref, _) in
                                                          enumerate(top_5)]) if top_5 else "Пока нет данных для топа!"

            markup = types.InlineKeyboardMarkup()
            btn_other1 = types.InlineKeyboardButton(
                f"За {'сутки' if other1 == 'day' else 'неделю' if other1 == 'week' else 'месяц'}",
                callback_data=f"top_{other1}")
            btn_other2 = types.InlineKeyboardButton(
                f"За {'сутки' if other2 == 'day' else 'неделю' if other2 == 'week' else 'месяц'}",
                callback_data=f"top_{other2}")
            btn_back = types.InlineKeyboardButton("Назад", callback_data="cancel")
            markup.row(btn_other1, btn_other2)
            markup.add(btn_back)

            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=top_text,
                                  reply_markup=markup, parse_mode='HTML')
            ...
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("Отмена", callback_data="admin_cancel"))
            bot.send_message(call.message.chat.id, "Введите ID канала (например, @ChannelName или -100123456789):",
                             reply_markup=markup)
            ...
            markup.add(types.InlineKeyboardButton("Отмена", callback_data="admin_cancel"))
            bot.send_message(call.message.chat.id,
                             f"Список каналов:\n{channels_list}\n\nВведи номер канала для удаления:",
                             reply_markup=markup)
            ...
            if not all_promos:
                ...
                return
            promo_list = "\n".join([
                                       f"{i + 1}. {p['code']} - {p['reward']} звезд ⭐️ (Лимит: {p['limit']}, Использовано: {len(p.get('used_by', []))})"
                                       for i, p in enumerate(all_promos)])
            admin_commands_table.insert({'user_id': user_id, 'step': 'delete_promo'})
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("Отмена", callback_data="admin_cancel"))
            bot.send_message(call.message.chat.id,
                             f"Список промокодов:\n{promo_list}\n\nВведите номер промокода который хотите удалить:",
                             reply_markup=markup)
            ...
            users_day = len(
                [u for u in users if now - datetime.fromisoformat(u['registration_date']) <= timedelta(days=1)])
            users_week = len(
                [u for u in users if now - datetime.fromisoformat(u['registration_date']) <= timedelta(weeks=1)])
            users_month = len(
                [u for u in users if now - datetime.fromisoformat(u['registration_date']) <= timedelta(days=30)])
            users_total = len(users)

            stars_day = sum(w['stars'] for w in withdrawals if
                            w['status'] == 'Звезды отправлены' and now - datetime.fromisoformat(
                                w['timestamp']) <= timedelta(days=1))
            stars_week = sum(w['stars'] for w in withdrawals if
                             w['status'] == 'Звезды отправлены' and now - datetime.fromisoformat(
                                 w['timestamp']) <= timedelta(weeks=1))
            stars_month = sum(w['stars'] for w in withdrawals if
                              w['status'] == 'Звезды отправлены' and now - datetime.fromisoformat(
                                  w['timestamp']) <= timedelta(days=30))
            stars_total = sum(w['stars'] for w in withdrawals if w['status'] == 'Звезды отправлены')
            ...
            if not all_tasks:
                ...
                markup.add(types.InlineKeyboardButton("Назад", callback_data="admin_back"))
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                      text="Заданий нет!", reply_markup=markup)
                return

            task = all_tasks[page - 1]
            now = datetime.now()
            completed_day = sum(1 for u in users_table.all() for t in u.get('completed_tasks', []) if
                                t == task.doc_id and now - datetime.fromisoformat(u['registration_date']) <= timedelta(
                                    days=1))
            completed_week = sum(1 for u in users_table.all() for t in u.get('completed_tasks', []) if
                                 t == task.doc_id and now - datetime.fromisoformat(u['registration_date']) <= timedelta(
                                     weeks=1))
            completed_month = sum(1 for u in users_table.all() for t in u.get('completed_tasks', []) if
                                  t == task.doc_id and now - datetime.fromisoformat(
                                      u['registration_date']) <= timedelta(days=30))
            completed_total = sum(
                1 for u in users_table.all() for t in u.get('completed_tasks', []) if t == task.doc_id)
            ...
            markup = types.InlineKeyboardMarkup()
            btn_prev = types.InlineKeyboardButton("⬅️",
                                                  callback_data=f"admin_task_stats_{page - 1}" if page > 1 else "none")
            btn_page = types.InlineKeyboardButton(f"{page}/{len(all_tasks)}", callback_data="none")
            btn_next = types.InlineKeyboardButton("➡️", callback_data=f"admin_task_stats_{page + 1}" if page < len(
                all_tasks) else "none")
            ...
            if all_tasks:
                task_text = (
                    f"Ссылка: {all_tasks[0]['link']}\n"
                    f"Награда: {all_tasks[0]['reward']} ⭐️\n\n"
                    "<b>Статистика</b>\n"
                    f"За сегодня: {sum(1 for u in users_table.all() for t in u.get('completed_tasks', []) if t == all_tasks[0].doc_id and datetime.now() - datetime.fromisoformat(u['registration_date']) <= timedelta(days=1))}\n"
                    f"За неделю: {sum(1 for u in users_table.all() for t in u.get('completed_tasks', []) if t == all_tasks[0].doc_id and datetime.now() - datetime.fromisoformat(u['registration_date']) <= timedelta(weeks=1))}\n"
                    f"За месяц: {sum(1 for u in users_table.all() for t in u.get('completed_tasks', []) if t == all_tasks[0].doc_id and datetime.now() - datetime.fromisoformat(u['registration_date']) <= timedelta(days=30))}\n"
                    f"За всё время: {sum(1 for u in users_table.all() for t in u.get('completed_tasks', []) if t == all_tasks[0].doc_id)}"
                )
                markup = types.InlineKeyboardMarkup()
                ...
                btn_next = types.InlineKeyboardButton("➡️", callback_data="admin_task_stats_2" if len(
                    all_tasks) > 1 else "none")
                btn_delete = types.InlineKeyboardButton("Удалить задание",
                                                        callback_data=f"admin_delete_task_{all_tasks[0].doc_id}")
                btn_back = types.InlineKeyboardButton("Назад", callback_data="admin_back")
                markup.row(btn_prev, btn_page, btn_next)
                markup.add(btn_delete)
                markup.add(btn_back)
                ...
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                      text="Заданий нет!", reply_markup=types.InlineKeyboardMarkup().add(
            ...
            if not all_users:
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                      text="Пользователей нет!", reply_markup=types.InlineKeyboardMarkup().add(
                        types.InlineKeyboardButton("Назад", callback_data="admin_back")))
                return
            ...
            btn_next = types.InlineKeyboardButton("➡️", callback_data=f"admin_users_{page + 1}" if page < len(
                all_users) else "none")
            btn_freeze = types.InlineKeyboardButton("Заморозить аккаунт", callback_data=f"freeze_{user['user_id']}")
            btn_unfreeze = types.InlineKeyboardButton("Разморозить аккаунт",
                                                      callback_data=f"unfreeze_{user['user_id']}")
            ...
            markup.add(btn_reset)
            markup.add(btn_back)
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=user_text,
                                  reply_markup=markup)
            ...
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("Отмена", callback_data="admin_cancel"))
            bot.send_message(call.message.chat.id, "Введите ID пользователя которого хотите заморозить:",
                             reply_markup=markup)
            ...
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("Отмена", callback_data="admin_cancel"))
            bot.send_message(call.message.chat.id, "Введите ID пользователя которого хотите разморозить:",
                             reply_markup=markup)
            ...
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("Отмена", callback_data="admin_cancel"))
            bot.send_message(call.message.chat.id, "Введите ID пользователя которого хотите обнулить аккаунт:",
                             reply_markup=markup)
            ...
            btn_add_promo = types.InlineKeyboardButton("Создать промокод", callback_data="admin_add_promo")
            btn_delete_promo = types.InlineKeyboardButton("Удалить промокод", callback_data="admin_delete_promo")
            btn_set_min_refs = types.InlineKeyboardButton("Установить мин. рефералов",
                                                          callback_data="admin_set_min_refs")
            btn_set_min_tasks = types.InlineKeyboardButton("Установить мин. заданий",
                                                           callback_data="admin_set_min_tasks")
            btn_set_ref_reward = types.InlineKeyboardButton("Установить награду за реферала",
                                                            callback_data="admin_set_ref_reward")
            btn_freeze = types.InlineKeyboardButton("Заморозить", callback_data="admin_freeze")
            btn_unfreeze = types.InlineKeyboardButton("Разморозить", callback_data="admin_unfreeze")
            ...
            markup.add(btn_reset)
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                  text="<b>Админ-панель</b>", reply_markup=markup, parse_mode='HTML')
            ...
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                  text="<b>Главное меню</b>", reply_markup=main_menu_markup(), parse_mode='HTML')
    except Exception as e:
...
def main_menu_markup():
    ...
    btn_profile = types.InlineKeyboardButton("[👥] МОЙ ЭКИПАЖ", callback_data="profile")
    btn_withdraw = types.InlineKeyboardButton("[ 🪐 💫 ВЫВЕСТИ ЗВЕЗДЫ 🚀 ]", callback_data="withdraw")
    btn_tasks = types.InlineKeyboardButton("[🛰] МОИ МИССИИ", callback_data="tasks_1")
    btn_promo = types.InlineKeyboardButton("[ 🌠 🎫 ПРОМОКОД 🔮 ]", callback_data="promo")
    btn_top = types.InlineKeyboardButton("[🏆] ГАЛАКТИЧЕСКИЙ ТОП", callback_data="top_day")
    markup.add(btn_referral)
    markup.row(btn_profile, btn_withdraw)
